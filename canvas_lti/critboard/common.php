...
function newRubric(input){
	...
	for(i=0;i<labels.length;i++){
		...
		$("#critboard").append('<div id="'+tabid+'" data-title="' + labels[i]+'" ></div>');
		$("#"+tabid).html(clone);
		//fixHeight("#"+tabid);
		$("#"+listid).sortable({
...
function addSubmission(sid){
	...
	$("div[id^=tab]").find("img[data-sid="+sid+"]").each(function(index, element) {
		...
		rubrics += subscore + ",";
        comment += $(this).closest("div").data("title") + ": " + subscore + "  ";
    });
	rubrics += "\r";
	//console.log(rubrics);
	var stu = new Array();
...
//$debug="";
	...
	//$debug .= $a['due_at']. "| " . $b['due_at']."\r" ;
			...
			return strcmp($b['due_at'],$a['due_at']);
		}
